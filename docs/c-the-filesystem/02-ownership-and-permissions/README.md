...
The /etc directory is owned by the root user and root group. As a user not in the root group you have the execute permission. The execute permission gives the user the permission to traverse a directory and acces metadata about its files.